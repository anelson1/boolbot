import { Client, ActionRowBuilder, EmbedBuilder, Role, SelectMenuBuilder, SelectMenuInteraction } from 'discord.js'
...
const generateMessageContent = (gameData: GameData, interaction: SelectMenuInteraction): { embed: EmbedBuilder; row: ActionRowBuilder } => {
	...
	const row = new ActionRowBuilder().addComponents(
		new SelectMenuBuilder().setCustomId('select').setPlaceholder('No Days').setMinValues(0).setMaxValues(7).addOptions(
			{
				label: 'Monday',
				value: 'monday',
			},
...
const dmRoleMembers = async (gameData: GameData, role: Role, interaction: SelectMenuInteraction): Promise<GameData> => {
